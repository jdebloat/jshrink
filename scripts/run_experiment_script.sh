...
cat ${WORK_LIST} |  while read item; do
	...
	#A two hour timeout
	timeout 7200 ${JAVA} -Xmx20g -jar ${DEBLOAT_APP} --tamiflex poa-2.0.3.jar --maven-project ${item_dir} -T --public-entry --main-entry --test-entry --prune-app --remove-methods --verbose 2>&1 >${temp_file} 
	exit_status=$?
