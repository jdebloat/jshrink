...
OUTPUT_LOG_DIR="${PWD}/all_transformations_with_tamiflex_and_jmtrace_only_libs_output_log"
...
cat ${WORK_LIST} |  while read item; do
	...
	#A 10 hour timeout
	timeout ${TIMEOUT} ${JAVA} -Xmx20g -jar ${DEBLOAT_APP} --jmtrace "${MTRACE_BUILD}" --tamiflex ${TAMIFLEX} --maven-project ${item_dir} -T --use-cache --public-entry --main-entry --test-entry --class-collapser --inline --remove-fields --remove-methods --log-directory "${ITEM_LOG_DIR}" --verbose 2>&1 >${temp_file} 
	exit_status=$?
